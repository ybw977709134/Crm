...
public class BluetoothService {
    ...
    private Button searchDevices = null;//收索蓝牙
    private ListView unbondDevicesListView = null;//未连接的蓝牙设备
    private ListView bondDevicesListView = null;//已连接的蓝牙设备
    ...
    private void addBondDevicesToListView() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            map.put("deviceName", bondDevices.get(i).getName());
            map.put("deviceAddress",bondDevices.get(i).getAddress());
            data.add(map);// 把item项的数据加到data中    
        }
        String[] from = { "deviceName","deviceAddress" };
            ...
            this.bondDevicesListView.setOnItemClickListener(new OnItemClickListener() {
                ...
                public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
                    BluetoothDevice device = bondDevices.get(arg2);
                     MyApplication.device_adress.clear();
                    MyApplication.device_adress.add(device.getAddress());
//                    Intent intent = new Intent();
//                    intent.setClassName(context, "com.onemeter.activity.PrintDataActivity");
//                    context.startActivity(intent);
    ...
    private void addUnbondDevicesToListView() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            map.put("deviceAddress",unbondDevices.get(i).getAddress());
            data.add(map);// 把item项的数据加到data中
        }
        String[] from = { "deviceName","deviceAddress"};
        int[] to = { R.id.undevice_name ,R.id.undevice_address};
    ...
    public BluetoothService(Context context, ListView unbondDevicesListView, ListView bondDevicesListView, Button switchBT, Button searchDevices,Activity activity) {
        this.context = context;
