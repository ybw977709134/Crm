...
import com.onemeter.service.BluetoothService;
...
public class BluetoothAction implements View.OnClickListener {
    ...
    public BluetoothAction(Context context, ListView unbondDevices, ListView bondDevices, Button switchBT, Button searchDevices, Activity activity) {
        ...
        this.bluetoothService = new BluetoothService(this.context, this.unbondDevices, this.bondDevices, this.switchBT, this.searchDevices,this.activity);
    }
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.searchDevices://收索设备
                this.searchDevices();
                  break;
            case R.id.return_Bluetooth_btn://返回键
                activity.finish();
                break;
            case R.id.openBluetooth_tb://打开蓝牙功能
                if (!this.bluetoothService.isOpen()) {
                    // 蓝牙关闭的情况
